...
- **`src/store.rs` — storage engine.** `Store` is single-threaded and not internally synchronized; the HTTP layer gives one bounded worker thread exclusive ownership. An advisory sibling `.wal.lock` permits only one writable Store for a WAL. Every standalone mutation and atomic `WriteBatch` gets one monotonically increasing `u64` commit sequence. Each WAL record is an independent `["KVWL":4][payload_len:u32 BE][payload][crc32:u32 BE]` pre-release frame; CRC32 covers magic, encoded length, and payload. The payload stores `[op:u8][sequence:u64 BE]` plus SET/DELETE/batch fields. Earlier unreleased layouts are intentionally unsupported; stable format versioning and migration begin before production use. The default durability ordering is **append complete WAL record + flush + `sync_data` before mutating the memtable**; `KVDB_DURABILITY=buffered` deliberately omits `sync_data`. Any uncertain storage write poisons that Store instance, which then rejects writes until reopened. Recovery decodes and verifies a complete frame before applying it; a physically incomplete trailing frame is discarded as a whole and the file is truncated to the last valid boundary before appending. Checksum failures, invalid magic, unknown op codes, zero/non-increasing sequences, oversized fields, and malformed complete records are hard `InvalidData` errors. The manifest stores exact `sequence`, `history_start`, and SSTable-name lines followed by a required CRC32 line; checksum verification occurs before metadata is accepted. Its total bytes, line bytes, and live-table count are bounded. Names must be canonical `<stem>-NNNNNN.sst` entries with unique generations. During open, each live table must be non-empty, no newer than the manifest sequence, and logically newer than the previous table by its persisted commit-sequence range; numeric filename generations need not be ordered because background compaction replaces an old prefix using a newer generation. `Store::get` and `Store::len` are fallible so SSTable I/O/corruption is never silently converted to an absent key. Newer WAL-only sequences replay on top of the manifest's durable sequence. The memtable is `BTreeMap<Vec<u8>, Vec<VersionedValue>>`, with versions ordered by commit sequence; key, byte, version, and WAL byte budgets independently trigger flush. `get_at` selects the newest version not newer than the requested sequence and rejects reads before the retained-history boundary. `snapshot()` copies the current logical state, while `snapshot_at(sequence)` reconstructs a historical state from retained versions. `begin_transaction()` adds a read-your-writes overlay and read set; `commit_transaction()` resolves each read/write key's latest stored sequence instead of retaining an unbounded conflict map, then persists the overlay as one `WriteBatch`. Independent keys can commit concurrently, while a changed read key causes `TransactionError::Conflict`. **Deletes remain versioned tombstones**, so historical reads before and after a deletion are both correct. Compaction uses a streaming k-way merge with one decoded record per input table. Automatic threshold compaction builds its replacement on a background thread; foreground flushes remain newer suffix tables, publication is manifest-atomic, and twice-threshold growth applies write backpressure. `compact_with_retention(sequence)` retains each key's boundary anchor plus newer versions and may discard obsolete tombstones. `compaction_metrics()` reports input/output bytes and versions, duration, peak merge buffers, and foreground stalls.

- **`src/sstable.rs` — on-disk sorted tables + flush.** When the memtable exceeds `KVDB_MEMTABLE_LIMIT` entries (default 1024), it is flushed to an immutable, key-sorted SSTable file `<stem>-NNNNNN.sst`. Pre-release tables start with `KVDBSST`; each key record stores a strictly increasing list of `[sequence, Set/Tombstone]` versions. Records are grouped into logical 64-key blocks, followed by a persisted sparse index, Bloom filter, and fixed footer. The index persists table-wide minimum/maximum commit sequences; each block entry contains its first key, byte range, record count, and CRC32. The index has a separate CRC32 covering its exact bytes plus `index_offset`, so its metadata, block checksums, and bounds cannot be silently changed. Cached, uncached, full-scan, and streaming reads all verify each touched block before returning its final result. The Bloom filter first rejects keys that definitely are not in the file, then possible hits binary-search the index and scan at most one block. Opening a table reads and verifies only its footer/index. Full-key operations such as `Store::len` explicitly scan table data rather than retaining a hidden dense index. Flush ordering remains **write+fsync the SSTable → atomically update the checksummed manifest (`<stem>.manifest`, temp+rename) → truncate the WAL → clear the memtable.** A crash between steps is safe: a half-written SSTable not yet in the manifest is an orphan and ignored, and un-truncated WAL data simply replays.
...
- Until the first stable release, old development-only on-disk formats may be replaced without migration. Before production use, assign stable format versions and require explicit coordinated migrations. Preserve the distinction between a physically incomplete final WAL frame (truncate it) and a complete corrupt frame (hard error).
