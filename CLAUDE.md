...
- **`src/http.rs` — REST layer.** `router(state)` builds the axum 0.8 router; the router is constructed here (not in the binary) specifically so integration tests can drive it via `tower`'s `oneshot` without binding a socket. `/health` is public; all `/v1/*` routes sit behind `auth` middleware (`route_layer`). `AppState` sends commands through a bounded Tokio channel to the Store-owning worker; adjacent mutations share one WAL flush/fsync, queue saturation returns `503`, and completed background compactions are published between commands. Credential comparison uses `constant_time_eq` to avoid timing leaks.
